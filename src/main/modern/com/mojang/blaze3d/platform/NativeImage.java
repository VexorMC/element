...
public final class NativeImage implements AutoCloseable {
	...
	public static NativeImage read(@NotNull NativeImage.Format format, int width, int height, int[] pixels) throws IOException {
		IntBuffer byteBuffer2 = MemoryUtil.memAllocInt(pixels.length);
		byteBuffer2.put(pixels);
		byteBuffer2.flip();

		if (!format.supportedByStb()) {
			throw new UnsupportedOperationException("Don't know how to read format " + format);
		} else if (MemoryUtil.memAddress(byteBuffer2) == 0L) {
			throw new IllegalArgumentException("Invalid buffer");
		} else {
			NativeImage var9;
			long l = MemoryUtil.memAddress(byteBuffer2);
			MEMORY_POOL.malloc(l, byteBuffer2.limit());
			var9 = new NativeImage(format, width, height, true, l);
			return var9;
		}
	}
	...
	public static enum Format {
		...
		BGRA(4, true, true, true, false, true, 0, 8, 16, 255, 24, true),
