...
public class GlCommandEncoder implements CommandEncoder {
			...
			for (RenderPass.Draw<T> draw : collection) {
				...
				if (biConsumer != null) {
					biConsumer.accept(object, (RenderPass.UniformUploader)(string, gpuBufferSlice) -> {
						if (glRenderPass.pipeline.program().getUniform(string) instanceof Uniform.Ubo ubo) {
							GL32.glBindBufferRange(35345, ubo.blockBinding(), ((GlBuffer)gpuBufferSlice.buffer()).handle, gpuBufferSlice.offset(), gpuBufferSlice.length());
						}
					});
				}
	...
	private boolean trySetup(GlRenderPass glRenderPass, Collection<String> collection) {
		...
		for (Entry<String, Uniform> entry2 : glProgram.getUniforms().entrySet()) {
			...
			if (uniform instanceof Uniform.Ubo ubo) {
			...
			} else if (uniform instanceof Uniform.Utb utb) {
				int var41 = utb.location();
				int var42 = utb.samplerIndex();
				TextureFormat var43 = utb.format();
				int var59 = utb.texture();
				int var44 = var59;
			...
			} else if (uniform instanceof Uniform.Sampler sampler) {
				int var46 = sampler.samplerIndex();
				int glTextureView2 = sampler.location();

				GlTextureView glTextureView2x = (GlTextureView) glRenderPass.samplers.get(string2);
				if (glTextureView2x == null) continue;
				...
				if ((glTexture.usage() & 16) != 0) {
					target = 34067;
					GL11.glBindTexture(target, glTexture.id);
				} else {
					target = 3553;
					GlStateManager._bindTexture(glTexture.id);
				}

				GlStateManager._texParameter(target, 33084, glTextureView2x.baseMipLevel());
				GlStateManager._texParameter(target, 33085, glTextureView2x.baseMipLevel() + glTextureView2x.mipLevels() - 1);
				glTexture.flushModeChanges(target);

			} else {
				throw new IllegalStateException("Unknown Uniform type: " + uniform.getClass());
			}
		}
