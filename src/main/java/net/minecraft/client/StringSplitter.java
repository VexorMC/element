...
public class StringSplitter {
	...
	public float stringWidth(@Nullable String string) {
		if (string == null) {
			...
			StringDecomposer.iterateFormatted(string, new Style(), (i, style, j) -> {
	...
	public float stringWidth(Text formattedText) {
		MutableFloat mutableFloat = new MutableFloat();
		StringDecomposer.iterateFormatted(formattedText, new Style(), (i, style, j) -> {
			mutableFloat.add(this.widthProvider.getWidth(j, style));
			return true;
		});
		return mutableFloat.floatValue();
	...
	@Nullable
	public Style componentStyleAtWidth(Text formattedText, int i) {
		StringSplitter.WidthLimitedCharSink widthLimitedCharSink = new StringSplitter.WidthLimitedCharSink(i);
		return (Style)formattedText.visit(
				(style, string) -> StringDecomposer.iterateFormatted(string, style, widthLimitedCharSink) ? Optional.empty() : Optional.of(style), new Style()
			)
			.orElse(null);
	}
	...
	public Text headByWidth(Text formattedText, int i, Style style) {
		final StringSplitter.WidthLimitedCharSink widthLimitedCharSink = new StringSplitter.WidthLimitedCharSink(i);
		return (Text)formattedText.visit(new Text.StyledContentConsumer<Text>() {
			private final ComponentCollector collector = new ComponentCollector();

			@Override
			public Optional<Text> accept(Style style, String string) {
				...
				if (!StringDecomposer.iterateFormatted(string, style, widthLimitedCharSink)) {
					...
					if (!string2.isEmpty()) {
						this.collector.append(Text.of(string2, style));
					}

					return Optional.of(this.collector.getResultOrEmpty());
				} else {
					if (!string.isEmpty()) {
						this.collector.append(Text.of(string, style));
					}
	...
	public List<Text> splitLines(String string, int i, Style style) {
		List<Text> list = Lists.<Text>newArrayList();
		this.splitLines(string, i, style, false, (stylex, ix, j) -> list.add(Text.of(string.substring(ix, j), stylex)));
		return list;
	}

	public List<Text> splitLines(Text formattedText, int i, Style style) {
		List<Text> list = Lists.<Text>newArrayList();
		this.splitLines(formattedText, i, style, (formattedTextx, boolean_) -> list.add(formattedTextx));
		return list;
	}

	public List<Text> splitLines(Text formattedText, int i, Style style, Text formattedText2) {
		List<Text> list = Lists.<Text>newArrayList();
		this.splitLines(
			formattedText, i, style, (formattedText2x, boolean_) -> list.add(boolean_ ? Text.composite(formattedText2, formattedText2x) : formattedText2x)
		);
		return list;
	}

	public void splitLines(Text formattedText, int i, Style style, BiConsumer<Text, Boolean> biConsumer) {
		...
		while (bl) {
			...
			for (StringSplitter.LineComponent lineComponent : flatComponents.parts) {
				...
				if (!bl4) {
					...
					boolean bl6 = bl5 || c == ' ';
					bl2 = bl5;
					Text formattedText2 = flatComponents.splitAt(j, bl6 ? 1 : 0, style2);
		...
		Text formattedText3 = flatComponents.getRemainder();
		if (formattedText3 != null) {
			biConsumer.accept(formattedText3, bl3);
		} else if (bl2) {
			biConsumer.accept(Text.EMPTY, false);
		}
	}
	...
	static class FlatComponents {
		...
		public Text splitAt(int i, int j, Style style) {
			...
			while (listIterator.hasNext()) {
				...
				if (!bl) {
					if (k > l) {
						...
						if (!string2.isEmpty()) {
							componentCollector.append(Text.of(string2, lineComponent.style));
						}
	...
	class LineBreakFinder implements FormattedCharSink {
		...
		private Style lineBreakStyle = new Style();
		private boolean hadNonZeroWidthChar;
		private float width;
		private int lastSpace = -1;
		private Style lastSpaceStyle = new Style();
	...
	@Environment(EnvType.CLIENT)
	static class LineComponent implements Text {
		...
		@Override
		public <T> Optional<T> visit(Text.ContentConsumer<T> contentConsumer) {
			return contentConsumer.accept(this.contents);
		}

		@Override
		public <T> Optional<T> visit(Text.StyledContentConsumer<T> styledContentConsumer, Style style) {
