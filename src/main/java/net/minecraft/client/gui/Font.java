...
public class Font {
	...
	private final Function<Identifier, FontSet> fonts;
	final boolean filterFishyGlyphs;
	private final StringSplitter splitter;

	public Font(Function<Identifier, FontSet> function, boolean bl) {
		this.fonts = function;
		this.filterFishyGlyphs = bl;
		this.splitter = new StringSplitter((i, style) -> this.getFontSet(MinecraftClient.DEFAULT_FONT).getGlyphInfo(i, this.filterFishyGlyphs).getAdvance(style.isBold()));
	}

	FontSet getFontSet(Identifier resourceLocation) {
	...
	public Font.PreparedText prepareText(String string, float f, float g, int i, boolean bl, int j) {
		...
		StringDecomposer.iterateFormatted(string, new Style(), preparedTextBuilder);
	...
	public String plainSubstrByWidth(String string, int i, boolean bl) {
		return bl ? this.splitter.plainTailByWidth(string, i, new Style()) : this.splitter.plainHeadByWidth(string, i, new Style());
	}

	public String plainSubstrByWidth(String string, int i) {
		return this.splitter.plainHeadByWidth(string, i, new Style());
	}

	public Text substrByWidth(Text formattedText, int i) {
		return this.splitter.headByWidth(formattedText, i, new Style());
	}

	public int wordWrapHeight(String string, int i) {
		return 9 * this.splitter.splitLines(string, i, new Style()).size();
	}

	public int wordWrapHeight(Text formattedText, int i) {
		return 9 * this.splitter.splitLines(formattedText, i, new Style()).size();
	}

	public List<FormattedCharSequence> split(Text formattedText, int i) {
		return this.splitter.splitLines(formattedText, i, new Style()).stream().map(text -> {
			if (MinecraftClient.getInstance().getLanguageManager().isRightToLeft()) {	
				return FormattedCharSequence.forward(text.asFormattedString(), text.getStyle());
			} else {
				return FormattedCharSequence.backward(text.asFormattedString(), text.getStyle());
			}
		}).toList();
	}

	public List<Text> splitIgnoringLanguage(Text formattedText, int i) {
		return this.splitter.splitLines(formattedText, i, new Style());
	}

	public boolean isBidirectional() {
		return MinecraftClient.getInstance().getLanguageManager().isRightToLeft();
	}
	...
	class PreparedTextBuilder implements FormattedCharSink, Font.PreparedText {
		...
		public boolean accept(int i, Style style, int j) {
			FontSet fontSet = Font.this.getFontSet(MinecraftClient.DEFAULT_FONT);
			...
			Formatting textColor = style.getColor();
		...
		public void visit(Font.GlyphVisitor glyphVisitor) {
			...
			if (ARGB.alpha(this.backgroundColor) != 0) {
				...
				bakedGlyph = Font.this.getFontSet(MinecraftClient.DEFAULT_FONT).whiteGlyph();
		...
		private int getTextColor(@Nullable Formatting textColor) {
			if (textColor != null) {
				int i = ARGB.alpha(this.color);
				int j = textColor.getColorIndex();
		...
		private int getShadowColor(Style style, int i) {
			Integer integer = style.getColor().getColorIndex();
			if (integer != null) {
