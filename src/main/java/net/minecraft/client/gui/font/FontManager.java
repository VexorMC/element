...
public class FontManager implements AutoCloseable, ResourceReloadListener {
	...
	public void reload(ResourceManager resourceManager) {
		...
		this.prepare(resourceManager, executor)
				.thenAcceptAsync(preparation -> this.apply(preparation, MinecraftClient.getInstance().profiler), executor)
				.whenComplete((unused, throwable) -> {
					if (throwable != null) {
						LOGGER.error("Error during font reload", throwable);
					}
	...
	private CompletableFuture<FontManager.Preparation> prepare(ResourceManager resourceManager, Executor executor) {
		...
		for (Entry<Identifier, List<Resource>> entry : FONT_DEFINITIONS.listMatchingResourceStacks(resourceManager).entrySet()) {
			Identifier resourceLocation = FONT_DEFINITIONS.fileToId((Identifier)entry.getKey());
	...
	private static Set<FontOption> getFontOptions(GameOptions options) {
		Set<FontOption> set = EnumSet.noneOf(FontOption.class);
		if (options.forcesUnicodeFont) {
...
//		}
	...
	private void apply(FontManager.Preparation preparation, Profiler profilerFiller) {
		...
		Set<FontOption> set = getFontOptions(MinecraftClient.getInstance().options);
		profilerFiller.swap("reloading");
		...
		profilerFiller.pop();
		if (!this.fontSets.containsKey(MinecraftClient.DEFAULT_FONT)) {
	...
	private static List<Pair<FontManager.BuilderId, GlyphProviderDefinition.Conditional>> loadResourceStack(List<Resource> list, Identifier resourceLocation) {
			...
			try {
				...
				try {
					...
					for (int i = list3.size() - 1; i >= 0; i--) {
						FontManager.BuilderId builderId = new FontManager.BuilderId(resourceLocation, resource.getResourcePackName(), i);
				...
				LOGGER.warn("Unable to load font '{}' in {} in resourcepack: '{}'", resourceLocation, "fonts.json", resource.getResourcePackName(), var13);
			}
		}
	...
	private FontSet getFontSetRaw(Identifier resourceLocation) {
		return (FontSet)this.fontSets.getOrDefault(resourceLocation, this.missingFontSet);
	}

	private FontSet getFontSetCached(Identifier resourceLocation) {
	...
	record BuilderResult(FontManager.BuilderId id, FontOption.Filter filter, Either<CompletableFuture<Optional<GlyphProvider>>, Identifier> result) {
		...
		public Optional<List<GlyphProvider.Conditional>> resolve(Function<Identifier, List<GlyphProvider.Conditional>> function) {
			return this.result
				.map(
					completableFuture -> (completableFuture.join()).map(glyphProvider -> List.of(new GlyphProvider.Conditional(glyphProvider, this.filter))),
					resourceLocation -> {
	...
	record UnresolvedBuilderBundle(Identifier fontId, List<FontManager.BuilderResult> builders, Set<Identifier> dependencies)
		...
		public UnresolvedBuilderBundle(Identifier resourceLocation) {
